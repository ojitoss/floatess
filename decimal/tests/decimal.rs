use std::str::FromStr;
use floatess::{DigitsStream, digits_storage};
            ...
            $(
                vec.push(
                    CaseOp::<Decimal<$storage>, Decimal<$storage>, Decimal<$storage>> {
                        lhs: Decimal::new($int_lhs, <$storage as TryFrom<&[u8]>>::try_from(&$stream_lhs).unwrap()),
                        rhs: Decimal::new($int_rhs, <$storage as TryFrom<&[u8]>>::try_from(&$stream_rhs).unwrap()),
                        expected: Decimal::new($int_exp, <$storage as TryFrom<&[u8]>>::try_from(&$stream_exp).unwrap()),
                        desc: $desc
                    }
                );
            )*
